...
namespace XueFu.EntLib
{
    ...
    #region StringHelper数据验证操作类

    public sealed class StringHelper
    {
        /// <summary>
        /// 比较字符串（含有"全部可用"的字符串类型，默认"全部可用"的代表字符为"0"）
        /// </summary>
        /// <param name="srcString">源字符串</param>
        /// <param name="compareString">对比字符串</param>
        /// <returns></returns>
        public static bool CompareStringContainSpecialValue(string srcString, string compareString)
        {
            return CompareStringContainSpecialValue(srcString, compareString, ',', "0");
        }
        ...
        /// <returns></returns>
        public static bool CompareStringContainSpecialValue(string srcString, string compareString, char splitChar)
        {
            return CompareStringContainSpecialValue(srcString, compareString, splitChar, "0");
        }

        /// <summary>
        /// 比较字符串（含有"全部可用"的字符串类型）
        /// </summary>
        /// <param name="srcString">源字符串</param>
        /// <param name="compareString">对比字符串</param>
        /// <param name="splitChar">分隔符</param>
        /// <param name="containValue">"全部可用"的代表字符</param>
        /// <returns></returns>
        public static bool CompareStringContainSpecialValue(string srcString, string compareString, char splitChar, string containValue)
        {
            string str1 = splitChar + srcString + splitChar;
            string str2 = splitChar + compareString + splitChar;
            containValue = splitChar + containValue + splitChar;
            if (str1.Contains(containValue) || str2.Contains(containValue))
                return true;
            else
                return CompareString(srcString, compareString, splitChar);
        }

        /// <summary>
        /// 比较单个字符串（字符数组srcstr中是否含有comparstr字符串）
        /// </summary>
        /// <param name="srcString">源字符串</param>
        /// <param name="compareString">对比字符串</param>
        /// <returns></returns>
        public static bool CompareSingleString(string srcString, string compareString)
        {
            return CompareSingleString(srcString, compareString, ',');
        }
        ...
        /// <param name="splitChar">分隔符</param>
        /// <returns></returns>
        public static bool CompareSingleString(string srcString, string compareString, char splitChar)
        {
            if (srcString == compareString || (splitChar + srcString + splitChar).IndexOf((splitChar + compareString + splitChar), StringComparison.Ordinal) >= 0)
            {
                return true;
            }
            return false;
        ...
        /// <summary>
        /// 比较字符串（字符数组srcstr中是否含有comparstr字符数组中的任何一个）
        /// </summary>
        /// <param name="srcString">源字符串</param>
        /// <param name="compareString">对比字符串</param>
        /// <returns></returns>
        public static bool CompareString(string srcString, string compareString)
        {
            return CompareString(srcString, compareString, ',');
        }
        ...
        /// <param name="splitChar">分隔符</param>
        /// <returns></returns>
        public static bool CompareString(string srcString, string compareString, char splitChar)
        {
            if (!string.IsNullOrEmpty(srcString) && !string.IsNullOrEmpty(compareString))
            {
                if (srcString.Equals(compareString)) return true;
                srcString = splitChar + srcString + splitChar;
                foreach (string str2 in compareString.Split(new char[] { splitChar }))
                {
                    if (srcString.IndexOf((splitChar + str2 + splitChar), StringComparison.Ordinal) >= 0)
                    {
                        return true;
                    }
                }
        ...
        /// <param name="splitChar">分隔符</param>
        /// <returns></returns>
        public static string EqualString(string str1, string str2, char splitChar)
        {
            string returnStr = string.Empty;
            str1 = splitChar + str1 + splitChar;
            foreach (string info in str2.Split(splitChar))
            {
                if (str1.IndexOf(splitChar + info + splitChar, StringComparison.Ordinal) >= 0)
                {
                    returnStr = returnStr + splitChar + info;
                }
            }
            if (returnStr.StartsWith(splitChar.ToString())) return returnStr.Substring(1);
            return returnStr;
        }

        /// <summary>
        /// 从字符串组合srcstr去掉字符串组合str1
        /// </summary>
        /// <param name="str1"></param>
        /// <param name="srcstr"></param>
        /// <returns></returns>
        public static string SubString(string srcString, string str1)
        {
            return SubString(srcString, str1, ',');
        }


        /// <summary>
        /// 从字符串组合srcString去掉字符串组合str1
        /// </summary>
        /// <param name="srcString"></param>
        /// <param name="str1"></param>
        /// <param name="splitChar"></param>
        /// <returns></returns>
        public static string SubString(string srcString, string str1, char splitChar)
        {
            if (!string.IsNullOrEmpty(str1))
            {
                srcString = splitChar + srcString + splitChar;
                foreach (string info in str1.Split(new char[]{ splitChar}, StringSplitOptions.RemoveEmptyEntries))
                {
                    string item = splitChar + info + splitChar;
                    if (srcString.IndexOf(item, StringComparison.Ordinal) != -1)
                    {
                        srcString = srcString.Replace(item, splitChar.ToString());
                    }
                }
        ...
        public static string Decode(string content, string key)
        {
            ...
            if (content != string.Empty)
            {
                try
                {
                    ...
                    {
                        inputBuffer[i] = byte.Parse(strArray[i], System.Globalization.NumberStyles.HexNumber);//
                    }
        ...
        public static string MD5(string content, int startPossition, int length)
        {
            ...
            return EncryptHelper.MD5(content).Substring(startPossition, length);
        }
        ...
        private static string PrepareKey(string key)
        {
            ...
            return EncryptHelper.MD5(key).Substring(12, 8);
        }
        ...
        /// <param name="str1">正则公式</param>
        /// <param name="str2">替换后的字符串</param>
        /// <returns></returns>
        ...
        /// <param name="pattern">要替换掉的字符串</param>
        ...
        {
            return EncryptHelper.SHA1(content);
        }
