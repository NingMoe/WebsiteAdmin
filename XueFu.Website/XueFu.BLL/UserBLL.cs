...
namespace XueFu.BLL
{
    public sealed class UserBLL
    {
        ...
        {
            string str = EncryptHelper.MD5(user.ID.ToString() + HttpContext.Current.Server.UrlEncode(user.Name) + HttpContext.Current.Server.UrlEncode(user.RealName) + user.GroupID.ToString() + Config.ReadConfigInfo().SecureKey + ClientHelper.Agent);
