<%@ Page Language="C#" MasterPageFile="MasterPage.Master" AutoEventWireup="true" CodeBehind="Menu.aspx.cs" Inherits="XueFu.Web.Admin.Menu" %>
<%@ Import Namespace="XueFu.BLL" %>
<%@ Import Namespace="XueFu.Model" %>
<asp:Content ID="MasterContent" ContentPlaceHolderID="ContentPlaceHolder" runat="server">
<div class="position"><img src="images/PositionIcon.png"  alt=""/>菜单列表</div>
<div  class="listBlock">
    <ul>
...
<asp:Repeater ID="RecordList" runat="server">
    <ItemTemplate>
        <tr class="listTableMain" onmousemove="changeColor(this,'#FFFDD7')" onmouseout="changeColor(this,'#FFF')">
            ...
            <td style="width:10%"><img src="images/icon/<%#Eval("MenuImage")%>-icon.gif" /></td>	
            <td style="width:10%"><%#Eval("OrderID")%></td>	
            <td style="width:20%">
                <a href="?Action=Up&ID=<%# Eval("ID") %>&FatherID=<%=fatherID %>"><img src="images/moveUp.gif" alt="上移" title="上移" /></a> 
                <a href="?Action=Down&ID=<%# Eval("ID") %>&FatherID=<%=fatherID %>"><img src="images/moveDown.gif" alt="下移" title="下移" /></a>
                <a href="javascript:pop('MenuAdd.aspx?ID=<%# Eval("ID") %>&FatherID=<%=fatherID %>',600,450,'菜单修改','MenuAdd<%# Eval("ID") %>')"><img src="images/edit.gif" alt="修改" title="修改" /></a> 
                <a href='?Action=Delete&ID=<%# Eval("ID") %>&FatherID=<%=fatherID %>' onclick="return check()"><img src="images/delete.gif" alt="删除" title="删除" /></a>
            </td>   
        </tr>
    </ItemTemplate>
</asp:Repeater>
