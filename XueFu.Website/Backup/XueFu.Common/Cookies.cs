            ...
            private static bool CheckCookies()
            {
                ...
                if (str != string.Empty)
                {
                    try
                    {
                        ...
                        if (FormsAuthentication.HashPasswordForStoringInConfigFile(str3 + str4 + str5 + str6 + Config.ReadConfigInfo().SecureKey + ClientHelper.Agent, "MD5").ToLower() == str2.ToLower())
                        {
                            return true;
                        }
